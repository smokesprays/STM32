...
extern char file_number;
extern uint8_t change_file;
//extern char kek[];
...
{     
  ...
  if ((HCD_IsDeviceConnected(&USB_OTG_Core) != 1) || (f_open(&file, kek, FA_CREATE_ALWAYS | FA_WRITE) != FR_OK))
  {
    /* Set ON Red LED */ 
    while(1)
    {
  ...
  { 
    ...
    if (Time_Rec_Base <= TIME_REC)
    {
      ...
      if (Switch ==1)
     {
      ...
      /* User button pressed */
      if ( Command_index != 1 || change_file == 1)
      {
        /* Stop recording */
        WaveRecorderStop();  
        Command_index = 0;
